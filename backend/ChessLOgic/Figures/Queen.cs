    ...
    public override MoveResult PossibleMove(ref IFigure?[][] board, (int, int) moveStartPosition, (int, int) moveEndPosition)
    {
        ...
        if (deltaX == deltaY)
        {
            ...
            if (board[endX][endY] == null || board[endX][endY].Color != figure.Color)
            {
                ...
                ChessLoggerService.Log("QUEEN moves", $"color: {figure.Color} from {ChessLoggerService.CoordToString((startY,startX))} to {ChessLoggerService.CoordToString((endY,endX))}");
